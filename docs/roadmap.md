...
| 9 | **A session ran silently degraded** (observed in a konexo/voltpulse session, 2026-07-17, on spec-kit machinery of the same shape as ours): the working tree sat on a `promote/*` branch that doesn't carry `.claude/skills/`, so the rendered skill layer and the current CLAUDE.md were simply *absent* — every command kept "working" without its machinery, and only a human noticed. DevFlow had the same hole: nothing verified the loop's own assets exist before starting. | script + prompt | Two halves. **Detection** (script): `devflow-preflight.sh` — mechanical assert (config, core scripts, checker agent, all 9 rendered commands in either `.claude/skills/` or `.claude/commands/` form) wired into `devflow-init.sh` and `devflow-flow.sh init`; a degraded tree now **blocks loudly** at loop entry; behavior-tested by test-21. **Prevention** (prompt): the CLAUDE.md protocol block forbids branch-switching in the DevFlow checkout — other-branch work goes to a separate `git worktree`, so the machinery-carrying tree never changes underfoot; frozen by test-15 | *(this change)* |
